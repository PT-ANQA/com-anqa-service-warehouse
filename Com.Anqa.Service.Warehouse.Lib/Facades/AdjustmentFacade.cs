...
namespace Com.Anqa.Service.Warehouse.Lib.Facades.AdjustmentFacade
{
    public class AdjustmentFacade : IAdjustmentDoc
    {
        ...
        public async Task<int> Create(AdjustmentDocs model, string username, int clientTimeZoneOffset = 7)
        {
            ...
            using (var transaction = this.dbContext.Database.BeginTransaction())
            {
                try
                {
                    string code = GenerateCode("ANQ-ADJ/INT");
                    var storages = GetStorage(model.StorageCode);

                    string inventoryMovementIn = GenerateCode("ANQ-TB/ADJ");
                    string inventoryMovementOut = GenerateCode("ANQ-KB/ADJ");
