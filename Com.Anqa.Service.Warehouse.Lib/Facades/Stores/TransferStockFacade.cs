...
using Com.Anqa.Service.Warehouse.Lib.ViewModels.TransferViewModel;
...
namespace Com.Anqa.Service.Warehouse.Lib.Facades.Stores
{
    public class TransferStockFacade : ITransferStock
    {
        ...
        public IQueryable<TransferStockReportViewModel> GetReportQuery(DateTime? datEVRom, DateTime? dateTo, string status, string code, int offset)
        {
            ...
            var Query = (from a in dbContext.TransferOutDocs
                         ...
                         where a.IsDeleted == false
                             ...
                             && b.Reference.Contains("ANQ-KB/RTT")
        ...
        public async Task<int> Create(TransferOutDocViewModel model, TransferOutDoc model2, string username, int clientTimeZoneOffset = 7)
        {
            ...
            using (var transaction = this.dbContext.Database.BeginTransaction())
            {
                try
                {
                    string codeOut = GenerateCode("ANQ-KB/RTT");
                    string packingList1 = GenerateCode("ANQ-KB/PLR");
                    string CodeIn = GenerateCode("ANQ-TB/BRT");
                    string packingList2 = GenerateCode("ANQ-KB/PLB");
                    string expCode = GenerateCode("ANQ-KB/EXP");
                    string expCode2 = GenerateCode("ANQ-KB/EXP");
                    string codetransferin = GenerateCode("ANQ-TB/BRT");
                    model2.Code = codeOut;
        ...
        public Tuple<List<TransferStockViewModel>, int, Dictionary<string, string>> ReadModel(int Page = 1, int Size = 25, string Order = "{}", string Keyword = null, string Filter = "{}")
        {
            var Query =  from a in dbContext.TransferOutDocs
                         join b in dbContext.SPKDocs on a.Code equals b.Reference
                         where a.Code.Contains("ANQ-KB/RTT") && b.DestinationName != "GUDANG TRANSFER STOCK"
