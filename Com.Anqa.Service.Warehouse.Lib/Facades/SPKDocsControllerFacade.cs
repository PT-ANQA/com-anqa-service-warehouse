...
namespace Com.Anqa.Service.Warehouse.Lib.Facades
{
    public class SPKDocsControllerFacade : ISPKDoc
    {
        ...
        public string GeneratePackingList() // nomor urut/anq-FN/bulan/tahun
        {
            var generatedNo = "";
            var date = DateTime.Now;
            var lastSPKDoc = dbContext.SPKDocs.OrderByDescending(entity => entity.Id).FirstOrDefault(entity => entity.PackingList.Contains("ANQ-FN"));
            ...
            if (lastSPKDoc != null)
            {
                ...
                generatedNo = $"{nomorUrut.ToString("0000")}/anq-FN/{date.ToString("MM")}/{date.ToString("yy")}";
            }
            else
            {
                generatedNo = $"0001/anq-FN/{date.ToString("MM")}/{date.ToString("yy")}";
            }
