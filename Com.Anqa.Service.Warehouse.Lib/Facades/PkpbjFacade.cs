...
namespace Com.Anqa.Service.Warehouse.Lib.Facades
{
    public class PkpbjFacade : IPkpbjFacade
    {
        ...
        public Tuple<List<SPKDocs>, int, Dictionary<string, string>> Read(int Page = 1, int Size = 25, string Order = "{}", string Keyword = null, string Filter = "{}")
        {
            ...
            //IQueryable<SPKDocs> Query = this.dbSet.Include(x=>x.Items).Where(x => !x.PackingList.Contains("ANQ-FN"));
        ...
        {
            //IQueryable<SPKDocs> Query = this.dbSet.Include(x=>x.Items).Where(x => !x.PackingList.Contains("ANQ-FN"));
            IQueryable<SPKDocs> Query = this.dbSet.Include(x => x.Items).Where(x => x.Reference.Contains("ANQ-KB/RTT"));
        ...
        public async Task<int> Create(SPKDocs model, string username, int clientTimeZoneOffset = 7)
        {
            ...
            using (var transaction = this.dbContext.Database.BeginTransaction())
            {
                try
                {
                    ...
                    string packingList = GenerateCode("ANQ-KB/PLB");
                    string code = GenerateCode("ANQ-PK/PBJ");
